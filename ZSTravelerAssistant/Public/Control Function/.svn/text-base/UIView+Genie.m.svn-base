                          ...
                          completion:(void (^)())completion
{
    ...
    CGFloat ix = xInset*destRect.size.width/self.bounds.size.width;
    CGFloat iy = yInset*destRect.size.height/self.bounds.size.height;
    CGRect marginedDestRect = CGRectInset(destRect, ix, iy);
    CGFloat endRectDepth = isEdgeVertical(edge) ? marginedDestRect.size.height : marginedDestRect.size.width;
    
    CGRect tRect = [self convertRect:CGRectInset(self.bounds, xInset, yInset) toView:self.superview];
    BCSegment aPoints = bezierEndPointsForTransition(edge, tRect);
...
- (NSArray *) sliceImage: (UIImage *) image toLayersAlongAxis: (BCAxis) axis
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        UIImage *sliceImage = [UIImage imageWithCGImage: imref
                                                  scale:image.scale
