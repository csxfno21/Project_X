...
- (void)handleBytesAvailable
{
	...
	// If zero bytes were read, wait for the EOF to come.
        ...
        //changed by jacky.ding let the asi save the download file
        if ([self downloadDestinationPath]) {
            float fileLength = [[self.responseHeaders objectForKey:@"Content-Length"]floatValue];
            
            //if the size of file less than 1K don't save it
            if ((fileLength < 1024) && (404 == self.responseStatusCode)) {
                return;
            }
			BOOL append = NO;
...
+ (NSDate *)dateFromRFC1123String:(NSString *)string
{
	...
	[formatter setDateFormat:[NSString stringWithFormat:@"MMM %@dd yyyy HH:mm%@ z",day,seconds]];
